...
/* =====================================================
   LEGACY SITEMAP PATHS — RETURN 410 GONE

   The site previously served Yoast-style sitemaps
   (post-sitemap1.xml, page-sitemap.xml, sitemap_index.xml, ...).
   Sitemaps are now generated by WordPress core at
   /wp-sitemap.xml (see robots.txt). The legacy paths no longer
   exist, but WordPress resolves them to the home page and
   answers HTTP 200 with HTML — a soft 200. Crawlers therefore
   keep treating a phantom sitemap as valid and re-report the
   stale URLs it once listed (Ahrefs Site Audit: "3XX redirect
   in sitemap").

   This returns a hard 410 Gone for those paths only. WordPress
   core sitemaps (/wp-sitemap*.xml) are explicitly excluded.
   ===================================================== */

function skeleton_wp_is_legacy_sitemap_request() {
    ...
    // Never interfere with WordPress core sitemaps.
    if ( 0 === strpos( $file, 'wp-sitemap' ) ) {
        return false;
    }

    // Legacy Yoast / generic sitemap file names, e.g.
    // post-sitemap1.xml, page-sitemap.xml, category-sitemap.xml,
    // author-sitemap.xml, sitemap_index.xml, sitemap-index.xml.
    if ( preg_match( '#^[a-z0-9_-]*sitemap[a-z0-9_-]*\.xml$#', $file ) ) {
        return true;
    }

    // Yoast XSL stylesheets that accompanied those sitemaps.
    if ( preg_match( '#^(main|nav)?sitemap\.xsl$#', $file ) ) {
        return true;
    }
...
function skeleton_wp_gone_legacy_sitemaps() {

    // /sitemap.xml is intentionally left alone: the server
    // redirects it to /wp-sitemap.xml.
    $path = wp_parse_url( $_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH );
    if ( is_string( $path ) && 'sitemap.xml' === strtolower( basename( $path ) ) ) {
        return;
    }

    if ( ! skeleton_wp_is_legacy_sitemap_request() ) {
        return;
    }
    ...
    nocache_headers();
    echo "410 Gone\n\nThis sitemap no longer exists. The current sitemap is https://deeprootsmag.org/wp-sitemap.xml\n";
    exit;
}
add_action( 'template_redirect', 'skeleton_wp_gone_legacy_sitemaps', 0 );
